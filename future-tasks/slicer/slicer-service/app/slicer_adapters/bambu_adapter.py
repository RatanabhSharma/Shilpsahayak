                ...
                try:
                    ...
                    logger.error(
                        ...
                        extra={"job_id": job_id or "bambu"},
                                ...
                                # MULTI-PLATE GENERIC CONVERSION PATH
                try:
                    ...
                    for p in plates:
                        ...
                        for prof in production_profiles:
                            usable_x = prof.get("usableBuildVolumeX") or prof.get("buildVolumeX", 256)
                            usable_y = prof.get("usableBuildVolumeY") or prof.get("buildVolumeY", 256)
                            usable_z = prof.get("usableBuildVolumeZ") or prof.get("buildVolumeZ", 256)
                            
                            if usable_x >= p["dimensions"]["x"] and \
                               usable_y >= p["dimensions"]["y"] and \
                               usable_z >= p["dimensions"]["z"]:
                                valid_printers.append(prof)
                        
                        if not valid_printers:
                            logger.error(f"Plate {p['plate_id']} dimensions exceed all production printers.")
                            return {
                                "success": False,
                                "error_code": "NO_ENABLED_PRODUCTION_PRINTERS",
                                "error": f"Plate {p['plate_id']} dimensions {p['dimensions']} exceed all usable production printer envelopes."
                            }
                            
                        # Sort to pick smallest volume
                        valid_printers.sort(key=lambda x: (x.get("usableBuildVolumeX") or x.get("buildVolumeX", 256)) *
                                                          (x.get("usableBuildVolumeY") or x.get("buildVolumeY", 256)) *
                                                          (x.get("usableBuildVolumeZ") or x.get("buildVolumeZ", 256)))
                        ...
                        machine_settings_path = find_bambu_resource_file("machine", selected_printer.get("machineProfileFile"))
