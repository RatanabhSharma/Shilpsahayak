...
def decide_route(
    ...
    *,
    ...
    # ------------------------------------------------------------------
    # 3MF, STEP, STP -> Manual Review Policy
    # ------------------------------------------------------------------
    if fmt in ("standard_3mf", "slicer_project_3mf", "step", "stp"):
        return _result(
            SlicerRoute.MANUAL_REVIEW,
            RouteReasonCode.MANUAL_AMBIGUOUS_PROJECT,
            f"Format '{fmt}' requires engineer review. Automatic quotation is disabled for this format.",
            "none",
            color_analysis=color_analysis,
            model_analysis=model_analysis,
        )
