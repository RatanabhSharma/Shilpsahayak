...
def resolve_production_printer_profile(config: Optional[Dict[str, Any]]) -> Optional[Dict[str, Any]]:
    ...
    optional_numeric_limits = ("usableBuildVolumeX", "usableBuildVolumeY", "usableBuildVolumeZ")
    if not all(isinstance(cfg.get(key), (int, float)) and cfg[key] > 0 for key in numeric_limits[:-1]):
        return None
    for key in optional_numeric_limits:
...
def process_slicing_job(job_id: str, file_path: str, params: Dict[str, Any]):
    ...
    try:
        ...
        if isinstance(raw_profiles_list, list) and raw_profiles_list:
            ...
            is_multi_3mf = JOBS[job_id].get("is_multi_plate", False) and file_ext == ".3mf"
            if is_multi_3mf:
                class DummyElig:
                    eligible = True
        ...
        # Populate active_envelope from authoritative usableBuildVolumeX/Y/Z or buildVolumeX/Y/Z in the DB
        bvx = production_profile.get("usableBuildVolumeX") or production_profile.get("buildVolumeX")
        bvy = production_profile.get("usableBuildVolumeY") or production_profile.get("buildVolumeY")
        bvz = production_profile.get("usableBuildVolumeZ") or production_profile.get("buildVolumeZ")
