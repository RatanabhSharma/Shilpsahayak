function SkeletonBlock({ className = '' }: { className?: string }) {
  return (
    <div
      className={`ss-skeleton overflow-hidden bg-[#e9e3d9] ${className}`}
      aria-hidden="true"
    />
  );
}
    ...
    <div className="grid gap-5 lg:grid-cols-[1.2fr_0.8fr]">
      ...
      <div className="grid gap-5 sm:grid-cols-3 lg:grid-cols-1">
        {Array.from({ length: 3 }).map((_, index) => (
          <div
            key={index}
            className="grid grid-cols-[112px_1fr] overflow-hidden border border-[#ded8ce] bg-white sm:grid-cols-1 lg:grid-cols-[140px_1fr]"
          >
            <SkeletonBlock className="aspect-square w-full" />
...
export function CategoryGridSkeleton({ count = 4 }: { count?: number }) {
  return (
    <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-4">
      {Array.from({ length: count }).map((_, index) => (
        <div key={index} className="relative overflow-hidden bg-[#ded8ce]">
          <SkeletonBlock className="aspect-[4/5] w-full" />
          <div className="absolute bottom-0 left-0 right-0 p-4 sm:p-5">
            <SkeletonBlock className="h-5 w-2/3 bg-[#cfc7bb]" />
          </div>
        </div>
      ))}
    </div>
      ...
      <div>
        ...
        <div className="mt-4 flex gap-3">
          {Array.from({ length: 4 }).map((_, index) => (
            <SkeletonBlock key={index} className="h-20 w-20 shrink-0" />
          ))}
        </div>
      </div>
