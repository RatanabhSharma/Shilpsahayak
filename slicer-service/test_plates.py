        ...
        with zipfile.ZipFile(target_3mf, "r") as zin:
            with zipfile.ZipFile(stripped_3mf, "w", compression=zipfile.ZIP_DEFLATED) as zout:
                for item in zin.infolist():
                    # Remove plate info, slice info, cut info
                    if "plate_" in item.filename or "slice_info" in item.filename or "cut_information" in item.filename or "filament_sequence" in item.filename:
                        continue
                    zout.writestr(item.filename, zin.read(item.filename))
        
        print("Created stripped plates 3MF.")
