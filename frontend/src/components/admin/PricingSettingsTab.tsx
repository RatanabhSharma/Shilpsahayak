...
import { DEFAULT_AMS_SLOTS } from '../../services/pricing/pricingConfig';
import {
...
export function PricingSettingsTab() {
  ...
  const [amsSlots, setAmsSlots] = useState<AmsSlotConfig[]>(
    storedData?.amsSlots || DEFAULT_AMS_SLOTS
  );
  ...
  useEffect(() => {
    if (storedData) {
      ...
      if (storedData.amsSlots && storedData.amsSlots.length > 0) {
        setAmsSlots(storedData.amsSlots);
      }
    }
  }, [storedData]);
      ...
      prev.map((s) => {
        ...
        if (field === 'materialId') {
          ...
          return {
            ...s,
            materialId: val,
            materialType: mat ? mat.name : s.materialType,
          };
        }
        return { ...s, [field]: val };
      })
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      await updateMutation.mutateAsync({
        ...
        amsSlots,
        productionPrinterProfiles: storedData.productionPrinterProfiles,
      });
      setShowSaved(true);
    ...
    <form onSubmit={handleSave} className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-7 space-y-6">
          ...
          {/* Workshop AMS Slots Configuration Card */}
          <div className="rounded-2xl border-2 border-line bg-white p-6 shadow-sm space-y-4">
            <div className="flex items-center justify-between border-b border-line pb-4">
              <div className="flex items-center gap-3">
                <div className="p-2.5 rounded-xl bg-shell border border-line text-accent">
                  <Box className="w-5 h-5" />
                </div>
                <div>
                  <h3 className="font-display font-bold text-lg text-ink">
                    Workshop AMS Slots & Loaded Filaments
                  </h3>
                  <p className="text-xs text-muted">
                    Configure physical Bambu/multi-material AMS slots available in your workshop and the active spools loaded.
                  </p>
                </div>
              </div>
              <span className="text-[10px] font-mono font-bold bg-amber-50 text-accent border border-accent/20 px-2 py-0.5 rounded">
                HARDWARE SLOTS
              </span>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-1">
              {amsSlots.map((slot) => {
                const currentMat = materials.find((m) => m.id === slot.materialId) || materials[0];
                  ...
                  <div
                    ...
                    className={`p-4 rounded-xl border transition-all space-y-3 ${
                      slot.active
                        ...
                        : 'border-dashed border-line/60 bg-shell/30 opacity-70'
                    }`}
                  >
                    {/* Header: Slot Badge and Active Toggle */}
                    <div className="flex items-center justify-between pb-2 border-b border-line/60">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-xs font-display font-bold text-ink">
                          {slot.active ? 'Loaded' : 'Empty / Offline'}
                        </span>
                      </div>
                      <button
                        type="button"
                        onClick={() => updateSlotField(slot.slotNumber, 'active', !slot.active)}
                        className={`px-2 py-0.5 rounded text-[10px] font-mono font-bold cursor-pointer transition-colors ${
                          slot.active
                            ? 'bg-emerald-50 text-emerald-700 border border-emerald-200'
                            : 'bg-stone-100 text-muted border border-line'
                        }`}
                      >
                    ...
                    <div>
                      ...
                      <select
                        ...
                        onChange={(e) => updateSlotField(slot.slotNumber, 'materialId', e.target.value)}
                        className="w-full px-2.5 py-1.5 font-mono text-xs border border-line rounded bg-white disabled:opacity-50"
                      >
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-2">
                        <label className="relative cursor-pointer shrink-0">
                          <span
                            className="w-7 h-7 rounded-lg border border-black/20 block shadow-2xs"
                          ...
                          <input
                            ...
                            disabled={!slot.active}
                            value={slot.colorHex}
                            onChange={(e) => updateSlotField(slot.slotNumber, 'colorHex', e.target.value)}
                        ...
                        <input
                          type="text"
                          disabled={!slot.active}
                          value={slot.colorName}
                          onChange={(e) => updateSlotField(slot.slotNumber, 'colorName', e.target.value)}
                          placeholder="Color name"
                          className="flex-1 px-2.5 py-1.5 font-mono text-xs border border-line rounded bg-white disabled:opacity-50"
                        />
                        <input
                          type="text"
                          disabled={!slot.active}
                          ...
                          onChange={(e) => {
                            ...
                            if (val.startsWith('#') && val.length <= 7) {
                              updateSlotField(slot.slotNumber, 'colorHex', val);
                            }
                          }}
                          className="w-20 px-2 py-1.5 font-mono text-xs uppercase border border-line rounded bg-white text-center disabled:opacity-50"
                        />
                      </div>
                    </div>

                    {/* Quick Presets from chosen Material */}
                    {slot.active && currentMat && currentMat.colors.length > 0 && (
                      <div>
                        <span className="text-[9px] font-mono text-muted block mb-1">
                          Select from {currentMat.name} catalog:
                        </span>
                        <div className="flex flex-wrap gap-1">
                          {currentMat.colors.map((c) => (
                            <button
                              key={c.name}
                              type="button"
                              onClick={() => {
                                updateSlotField(slot.slotNumber, 'colorName', c.name);
                                updateSlotField(slot.slotNumber, 'colorHex', c.hex);
                              }}
                              title={`${c.name} (${c.hex})`}
                              className={`w-5 h-5 rounded border transition-transform hover:scale-110 cursor-pointer ${
                                slot.colorHex.toUpperCase() === c.hex.toUpperCase()
                                  ? 'ring-2 ring-accent ring-offset-1 border-black/40'
                                  : 'border-black/20'
                              }`}
