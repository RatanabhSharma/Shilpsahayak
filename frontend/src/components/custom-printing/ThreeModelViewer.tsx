...
import { ParsedModelResult, ColorReplacement } from '../../services/model/modelTypes';
...
export interface ThreeModelViewerProps {
  ...
  colorHex?: string;
  colorReplacements?: Record<number, ColorReplacement>;
...
export const ThreeModelViewer: React.FC<ThreeModelViewerProps> = ({
  ...
  useEffect(() => {
    ...
    if (!displayObj) {
      if (object3d) {
        ...
        if (geometry.hasAttribute('filamentSlot')) {
          clonedGeometry.setAttribute('filamentSlot', geometry.getAttribute('filamentSlot'));
        }
        if (geometry.hasAttribute('originalColor')) {
          clonedGeometry.setAttribute('originalColor', geometry.getAttribute('originalColor'));
        } else if (geometry.hasAttribute('color')) {
          clonedGeometry.setAttribute('originalColor', geometry.getAttribute('color').clone());
        }
  ...
  // 2. Update Materials & Live Color Replacements (Runs on colorMode, colorReplacements, isMultiColor, colorHex, isWireframe)
  useEffect(() => {
    ...
    // Fast color lookup maps for replacements
    const replacementColors = new Map<number, THREE.Color>();
    const replacementHexes = new Map<string, THREE.Color>();
    if (colorReplacements) {
      Object.values(colorReplacements).forEach((rep) => {
        const col = new THREE.Color(rep.productionHex);
        replacementColors.set(rep.sourceFilament, col);
        replacementHexes.set(rep.originalHex.toUpperCase(), col);
      });
    }
    ...
    displayObj.traverse((child) => {
      ...
      const originalColorAttr =
        (geom?.getAttribute('originalColor') as THREE.BufferAttribute | undefined) || colorAttr;
      const filamentSlotAttr = geom?.getAttribute('filamentSlot') as THREE.BufferAttribute | undefined;
      const hasVertexColors = Boolean(colorAttr && colorAttr.count > 0);

      if (hasVertexColors && (isMultiColor || hasOriginalColors)) {
        // Painted multicolor model with per-vertex colors
        // Keep original material with vertexColors enabled
        ...
        if (colorAttr && originalColorAttr) {
          const count = colorAttr.count;

          if (isOriginalMode) {
            // "Model Colours": Restore 100% original model vertex colors
            ...
            // "Filament Colour": Apply replacements per filament slot (or keep original if unreplaced)
            const tempColor = new THREE.Color();
            for (let i = 0; i < count; i++) {
              const slot1Based = filamentSlotAttr ? Math.round(filamentSlotAttr.getX(i)) : 0;
              let repColor = replacementColors.get(slot1Based);

              if (!repColor && replacementHexes.size > 0) {
                tempColor.setRGB(
                  originalColorAttr.getX(i),
                  originalColorAttr.getY(i),
                  originalColorAttr.getZ(i)
                );
                const hex = `#${tempColor.getHexString().toUpperCase()}`;
                repColor = replacementHexes.get(hex);
              }

              if (repColor) {
                colorAttr.setXYZ(i, repColor.r, repColor.g, repColor.b);
              } else {
                colorAttr.setXYZ(
                  i,
                  originalColorAttr.getX(i),
      ...
      } else if (m.userData?.sourceFilament && isMultiColor) {
        // Multi-mesh assembly
        const slot = m.userData.sourceFilament;
        const originalHex = m.userData.originalHex || '#AAAAAA';
        const rep = colorReplacements?.[slot];
        const targetHex = isOriginalMode ? originalHex : (rep?.productionHex || originalHex);

        if (m.material instanceof THREE.MeshStandardMaterial) {
          m.material.color.set(targetHex);
          m.material.wireframe = isWireframe;
          m.material.needsUpdate = true;
        }
      } else {
        // Single material / standard model
        if (isOriginalMode) {
          const orig = originalMaterialsMapRef.current.get(m);
          if (orig) {
            m.material = orig;
  ...
  }, [colorMode, colorReplacements, isMultiColor, colorHex, isWireframe, hasOriginalColors]);
