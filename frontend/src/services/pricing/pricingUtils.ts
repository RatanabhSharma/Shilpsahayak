...
/**
 * @deprecated PROHIBITED IN PRODUCTION ORDER QUOTATION PATH.
 * Production orders must use actual slicer toolpath weight, not heuristics.
 *
 * For INSTANT CUSTOMER ESTIMATES, use `computeInstantEstimate()` in
 * `src/services/pricing/instantEstimator.ts` instead — it applies a
 * geometry-based heuristic correctly and clearly labels the result as an estimate.
 */
 ...
 *
 * For INSTANT CUSTOMER ESTIMATES, use `computeInstantEstimate()` in
 * `src/services/pricing/instantEstimator.ts` instead.
 */
