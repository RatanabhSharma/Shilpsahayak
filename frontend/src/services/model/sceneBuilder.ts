...
export function buildThreeGeometry(parsed: ParsedGeometry): THREE.BufferGeometry {
  ...
  if (parsed.originalColor && parsed.originalColor.length > 0) {
    geom.setAttribute('originalColor', new THREE.BufferAttribute(parsed.originalColor, 3));
  } else if (parsed.color && parsed.color.length > 0) {
    geom.setAttribute('originalColor', new THREE.BufferAttribute(new Float32Array(parsed.color), 3));
  }

  if (parsed.filamentSlots && parsed.filamentSlots.length > 0) {
    geom.setAttribute('filamentSlot', new THREE.BufferAttribute(parsed.filamentSlots, 1));
    geom.userData.filamentSlots = parsed.filamentSlots;
  }
...
export function bufferGeometryToParsedGeometry(geom: THREE.BufferGeometry): ParsedGeometry {
  ...
  const origColAttr = geom.attributes.originalColor || geom.attributes.color;
  const originalColor = origColAttr ? new Float32Array(origColAttr.array) : undefined;

  const slotAttr = geom.attributes.filamentSlot;
  const filamentSlots = slotAttr
    ? new Uint8Array(slotAttr.array)
    : (geom.userData?.filamentSlots ? new Uint8Array(geom.userData.filamentSlots) : undefined);
  ...
  return { position, normal, color, originalColor, uv, index, filamentSlots };
}
...
export function buildMeshForObject(
  ...
  if (obj.sourceFilament !== undefined) {
    mesh.userData.sourceFilament = obj.sourceFilament;
  }
  if (obj.color) {
    mesh.userData.originalHex = obj.color;
  }
