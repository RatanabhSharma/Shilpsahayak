...
export type BambuParseResult =
  | { success: true; geometry: THREE.BufferGeometry; colors: string[]; hasColors: boolean; filamentSlots?: Uint8Array }
...
export function applyBambuProjectMaterials(
  ...
  for (let i = 0; i < meshes.length; i++) {
    ...
    mesh.material = mat;
    mesh.userData.sourceFilament = slot + 1;
    mesh.userData.originalHex = hex;
  }
  ...
  try {
    ...
    for (let f = 0; f < faceColors.length; f++) {
      ...
      for (let v = 0; v < 3; v++) {
        ...
        flatFilamentSlots[f * 3 + v] = slot1Based;
      }
    }
    ...
    geometry.setAttribute(
      'originalColor',
      new THREE.Float32BufferAttribute(new Float32Array(flatColors), 3)
    );
    geometry.setAttribute(
      'filamentSlot',
      new THREE.Uint8BufferAttribute(flatFilamentSlots, 1)
    );
    geometry.userData.filamentSlots = flatFilamentSlots;
    geometry.userData.originalColors = filamentColors;
    ...
    return {
      ...
      hasColors,
      filamentSlots: flatFilamentSlots,
    };
  } catch (err: any) {
